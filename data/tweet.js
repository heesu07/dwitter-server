...
const INCLUDE_USER = {
  attributes: [
    ...
    'createdAt',
    'userId',
    [Squelize.col('user.name'), 'name'],
    [Squelize.col('user.username'), 'username'],
    [Squelize.col('user.url'), 'url'],
  ],
  include: {
    model: User,
    attributes: [],
...
export async function create(text, userId) {
  ...
  .then(data => this.getById(dat.dataValues.id));
}
